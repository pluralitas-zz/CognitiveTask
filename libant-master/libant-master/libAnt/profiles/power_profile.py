...
class PowerProfileMessage(ProfileMessage):
    ...
    def pedalBalanceValue(self):
        ...
        '''#check if 8 bits
        if(len(cBinary[2:])>6):
            #eliminate the "0b" & 8th bit(0-2bit)
            cBinary_remove=cBinary[3:]               
            #BinArray->int
            cBinary2=int(cBinary_remove,2)
            
            #---all print Right Pedal----
            #if 1->Right Pedal
            if ((cBinary[2] == "1") ):
                print("R1")
                return (cBinary2/255*100)
            #if 0->Left Pedal ==> 100%-Left Pedal=Right Pedal 
            elif ( ( cBinary[2] == "0")): 
                print("R2")
                return (100-(cBinary2/255*100))

        #7bits or less
        else:
            #eliminate the "0b"
            cBinary_remove0b=cBinary[2:]
            #str->int
