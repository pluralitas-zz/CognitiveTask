...
class PowerProfileMessage(ProfileMessage):
    ...
    def __str__(self):
        if self.dataPageNumber == 16 :   #Frances
            return '{0:.0f},'.format(self.instantaneousPower) + '{0:.0f},'.format(self.averagePower) + '{0:.0f}, '.format(self.instantaneousCadence) + '{0:.0f}'.format(self.pedalBalanceValue)
            
        else: #self.dataPageNumber == 19: #data page 0X13    #Frances
                return  '{0:.0f},'.format(self.LeftTorqueEffectiveness) + '{0:.0f}'.format(self.RightTorqueEffectiveness)
