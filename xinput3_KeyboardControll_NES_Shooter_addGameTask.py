...
def sample_first_joystick():
    ...
    def on_button(button, pressed):
        ...
        if pressed == 1: #when pressed down
            if button == 1: #Up
            ...
            elif button == 13: # PS-Cross
                keyboard.press('c')            
                time.sleep(1)
                keyboard.release('c') 
            elif button == 14: #PS-Circle
                keyboard.press('v')            
                time.sleep(1)
                keyboard.release('v') 
            elif button == 16: #PS-Triangle
                keyboard.press('f')            
                time.sleep(1)
                keyboard.release('f') 
            elif button == 15: #PS-Square
                keyboard.press('d')            
                time.sleep(1)
                keyboard.release('d') 
            elif button == 9: #Left Shoulder
                keyboard.press('e')            
                time.sleep(1)
                keyboard.release('e') 
            elif button == 15:  #Right Sholder
                keyboard.press('r')            
                time.sleep(1)
                keyboard.release('r') 
        # if pressed == 1:
        #     print('Pressed')
            #X = input()
            #Map controller ABXY to k eypress ABXY
            #button13=B butoon14=A button15=Y button16=X
    ...
    while True:
        ...
        time.sleep(.01)
'''
#threading for encorder speed
#Press'b' to accerelat if speed>4rpm
#Press 'a' to decelerat if speed<0rpm
def thread_SPD(enc):
    #x_prev = 0
    while 1:
        #read speed
...
def thread_GUI():
    ...
    root.mainloop()      
'''
def initController(speed):
    #Initialize keyboard
    keyboard = Controller()
    #Initialize & start Encoder thread
    th_SPD = threading.Thread(target=thread_SPD, args=(speed,), daemon=True)
    th_SPD.start()
    #Create a Game Joystick
    sample_first_joystick()
    print("2")
'''
if __name__ == "__main__":
    global enc
    #create keyboard object for game controller mapping
    keyboard = Controller()
    #Initialize Encoder
    enc = serial.Serial(port='COM9',baudrate=38400,parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE,bytesize=serial.EIGHTBITS,rtscts=True) #Optical Encoder config E1090BK25 chang chun hua te guang dian
    #Initialize & start Encoder thread
    th_SPD = threading.Thread(target=thread_SPD, args=(enc,), daemon=True)
    th_SPD.start()
    #th_SPD.join()

    #Initialize & start GUI thread
    th_GUI=threading.Thread(target=thread_GUI,args=(), daemon=True)
    th_GUI.start()
